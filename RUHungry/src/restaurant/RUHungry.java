...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        for(int i=0;i<categories;i++){
            ...
            int Totdishes = StdIn.readInt();
            for(int j=0;j<Totdishes;j++){
                StdIn.readLine();
                String Dish_name= StdIn.readLine();
                int[] StockID=new int[StdIn.readInt()];
                for(int k=0;k<StockID.length;k++){
                    StockID[k]=StdIn.readInt();
                }
                Dish dish = new Dish(category, Dish_name, StockID);
                MenuNode menu = new MenuNode(dish, menuVar[i]);
                menuVar[i]=menu;
            }
        }
    }
    ...
    public MenuNode findDish ( String dishName ) {
        ...
        for ( int category = 0; category < menuVar.length; category++ ) {
            ...
            while ( ptr != null ) { // while loop that searches the LL of the category to find the itemOrdered
                if ( ptr.getDish().getName().equalsIgnoreCase(dishName) ) return ptr;
                else ptr = ptr.getNextMenuNode();
            }
        }
        return menuNode;
    }
    ...
    public void addStockNode ( StockNode newNode ) {
        int idx=newNode.getIngredient().getID()% stockVarSize;
        if(stockVar[idx]==null) stockVar[idx]=newNode;
        else {
            newNode.setNextStockNode(stockVar[idx]);
            stockVar[idx]= newNode;
        }
}
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        Ingredient nodeToUpdate = null;
        if (ingredientName != null) {
            nodeToUpdate = findStockNode(ingredientName).getIngredient();
        }
        else if (ingredientID != -1) {
            nodeToUpdate = findStockNode(ingredientID).getIngredient();
        }
        nodeToUpdate.updateStockLevel(stockAmountToAdd);
    }
    ...
    public void updatePriceAndProfit() {
        for (int i = 0; i < menuVar.length; i++) {
            ...
            while (menuNode!=null) {            
                ...
                int StkID[]=menuNode.getDish().getStockID();
                for (int j = 0; j < StkID.length; j++) {
                    double ingredientCost = findStockNode(StkID[j]).getIngredient().getCost();
                    totalCost += ingredientCost;
                }
                double finalPrice = totalCost * 1.2;

                menuNode.getDish().setPriceOfDish(finalPrice);
                menuNode.getDish().setProfit(finalPrice - totalCost);
    ...
    public void addTransactionNode ( TransactionData data ) {
        TransactionNode NodeThatINeedToAddToTheEndOfMyLinkedListSoThatMyTransactionIsRecorded=new TransactionNode(data,null);
        if(transactionVar==null){
            transactionVar= NodeThatINeedToAddToTheEndOfMyLinkedListSoThatMyTransactionIsRecorded;
            return;
        }
        else{
            TransactionNode tmp=transactionVar;
            while (tmp.getNext()!=null) {
                tmp=tmp.getNext();
            }
            tmp.setNext(NodeThatINeedToAddToTheEndOfMyLinkedListSoThatMyTransactionIsRecorded);
        }
    }
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        for(int i=0; i<findDish(dishName).getDish().getStockID().length; i++){
            if(findStockNode(findDish(dishName).getDish().getStockID()[i]).getIngredient().getStockLevel()< numberOfDishes) return false;
        }
        return true;
    }
     ...
     */
     ...
     public void order (String dishName, int quantity){
            if(checkDishAvailability(dishName, quantity)){
                TransactionData dataOfTransaction= new TransactionData("order", dishName, quantity, quantity*findDish(dishName).getDish().getProfit(), true);
                addTransactionNode(dataOfTransaction);
                int[] StkID= findDish(dishName).getDish().getStockID();
                for(int k=0;k< StkID.length;k++){
                    String ingredientName= findStockNode(StkID[k]).getIngredient().getName();
                    updateStock(ingredientName, StkID[k], -quantity);
                }
            }
            else{
                TransactionData dataOfTransaction= new TransactionData("order", dishName, quantity, 0, false);
                addTransactionNode(dataOfTransaction);
                MenuNode ptr= findDish(dishName).getNextMenuNode();
                while(ptr!= null){
                    if(checkDishAvailability(ptr.getDish().getName(), quantity)){ 
                        TransactionData substitution= new TransactionData("order", ptr.getDish().getName(),quantity,quantity*ptr.getDish().getProfit(), true);
                        addTransactionNode(substitution);
    
                        int[] StkID= findDish(ptr.getDish().getName()).getDish().getStockID();
                    for(int k=0;k< StkID.length;k++){
                        String ingredientName= findStockNode(StkID[k]).getIngredient().getName();
                        updateStock(ingredientName, StkID[k], -quantity);
                    }
                    return;
                        ...
                        TransactionData failedSub= new TransactionData("order", ptr.getDish().getName(), quantity, 0, false);
                        addTransactionNode(failedSub);
                    }
                    ptr= ptr.getNextMenuNode();
                }
                int index = findCategoryIndex(findDish(dishName).getDish().getCategory());
                MenuNode frontPtr= menuVar[index];
                if(frontPtr.getDish()== findDish(dishName).getDish()){
    
                    return;
                }
                else{
                while(frontPtr != null && frontPtr!= findDish(dishName)){
                    if(checkDishAvailability(frontPtr.getDish().getName(), quantity)){
                        TransactionData substitution= new TransactionData("order", frontPtr.getDish().getName(),quantity,quantity*frontPtr.getDish().getProfit(), true);
                        addTransactionNode(substitution);
                        
                        int[] StkID= findDish(frontPtr.getDish().getName()).getDish().getStockID();
                    for(int k=0;k< StkID.length;k++){
                        String ingredientName= findStockNode(StkID[k]).getIngredient().getName();
                        updateStock(ingredientName, StkID[k], -quantity);
                    }
                    return;
                        ...
                        TransactionData failedFromFront= new TransactionData("order", frontPtr.getDish().getName(), quantity, 0, false);
                    addTransactionNode(failedFromFront);
                    }
    ...
    public double profit () {
        ...
        while(tmp!=null){
            totProfit+= tmp.getData().getProfit();
            tmp=tmp.getNext();
        }
        return totProfit;
    }
    ...
    public void donation(String ingredientName, int quantity){
        ...
        boolean isDonationSuccessful = profit() > 50.0 && stockNode.getIngredient().getStockLevel() >= quantity;
        TransactionData donationData = new TransactionData("donation", ingredientName, quantity, 0, isDonationSuccessful);
        addTransactionNode(donationData);
    ...
    public void restock(String ingredientName, int quantity){
        ...
        if(profit()< quantity*Cost){
            TransactionData FRestock= new TransactionData("restock", ingredientName, quantity, 0, false);
            addTransactionNode(FRestock);
        }
        else{
            TransactionData successfulRestock= new TransactionData("restock", ingredientName, quantity, -quantity*Cost, true);
